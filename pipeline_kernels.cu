...
// grayscale kernel: converts bgr to gray using standard weights
...
// wipe transition kernel: splits the screen dynamically between raw and filtered frames
__global__ void wipe_kernel(unsigned char* in_a, unsigned char* in_b, unsigned char* out, int w, int h, float t) {
    ...
    if (x < w && y < h) {
        ...
        if (x < boundary) {
            out[idx] = in_b[idx]; 
            out[idx+1] = in_b[idx+1]; 
            out[idx+2] = in_b[idx+2];
        } else {
            out[idx] = in_a[idx]; 
            out[idx+1] = in_a[idx+1]; 
            out[idx+2] = in_a[idx+2];
        }
    }
}

// external wrappers linkable via c++ main script
